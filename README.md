Hi 👋 I’m Ajad Singh Dinkar — Web developer and Competitive Programmer
 
💻 Interests: Full-Stack Development, AI/ML & Problem Solving  
🏆 Competitive Programmer with top global rankings  

**Tech:** C/C++, Java, Python | MERN, Next.js, Flask | TensorFlow, PyTorch  
**Databases & Cloud:** MongoDB, PostgreSQL | Docker, AWS  

**Experience:**  
- Full Stack Intern @ BillFree Labs — REST APIs, React UI  
- AI/ML Intern @ Next24tech — ML models (92% accuracy)

**Projects:** TalentBridge (Full-Stack) | SpotTrack (AI Object Tracking)

**Achievements:** Expert @ Codeforces | Knight @ LeetCode | ICPC India 2025 Rank 742  

📫 azadsinghdinkar@gmail.com | LinkedIn | GitHub
